...
当前发布版本为 `0.3.0`；本版本的架构图示覆盖仓库维护、资产盘点、候选归并、受控修复、行为等价验证和多前置版本语义连续性闭环。

- [ADR 0001](../../../adr/0001-multi-host-layered-enforcement.md) 记录为什么采用“Skill 判断、Hook 接线、校验器与 CI 阻断”的架构决策；
- [ADR 0002](../../../adr/0002-semantic-consolidation.md) 记录为什么采用资产盘点、候选归并、受控清理和人工裁决闭环；
...
`0.3.0` 将该能力作为正式合并门禁交付，架构主流程、开发流程和下方多分支时序图共同描述其执行位置与失败行为。
