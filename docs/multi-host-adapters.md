...
`0.3.0` 继续采用一个共享语义控制面和三个宿主适配器。宿主适配器只负责发现、生命周期输入输出和安装投影，不复制 Skill 正文或
语义对象模型。
...
`runtime/route.mjs` 选择 `bootstrap`、`maintenance`、`fast`、`standard`、`strict` 或 `idle` 路由；
`semantic-status` 汇总路由、基线闭合、预检和 Frontier。
路由状态同时保存静态 `capabilities`、本次 `runtimeProbe`、真实事件产生的 `hookEvidence` 和成功 Stop 的短期 `verificationReceipt`。仅探测到宿主不会启用门禁；
...
- Stop：有变化时运行严格语义校验；Codex 与 Claude Code 失败时阻断，Cursor 失败时发送 `followup_message` 并保留预检，成功消费它；同一未变化工作树的重入复用成功收据。
