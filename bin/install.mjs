...
function pluginHook(group) {
  ...
  return typeof command === "string" && command.includes(pluginId);
}
...
function installCodex(dryRun) {
  ...
  bestEffort(
    command,
    ["plugin", "remove", `${pluginId}@${marketplace}`, "--json"],
    dryRun,
  );
  bestEffort(
    command,
...
function installClaude(dryRun) {
  ...
  bestEffort(
    command,
    ["plugin", "uninstall", `${pluginId}@${marketplace}`, "--scope", "user"],
    dryRun,
  );
  bestEffort(command, ["plugin", "marketplace", "remove", marketplace], dryRun);
  run(
    command,
...
function cursorTarget() {
  return join(homedir(), ".cursor", "plugins", "local", pluginId);
}
